...
def mean_confusion_matrix(
    y_pred: typing.List[np.ndarray], y_true: typing.List[np.ndarray], **kwargs
) -> np.ndarray:
    """
    This function allows you to calculate the mean of
    confusion matrices based on multiple y_pred and y_true arrays.

    Arguments
    ---------

    - y_pred: typing.List[np.ndarray]:
        A list arrays of the predicted values.

    - y_true: typing.List[np.ndarray]:
        A list arrays of the true values.

    - **kwargs:
        Other keyword arguments are passed to the
        :code:`sklearn.metrics.confusion_matrix` function.

    Returns
    ---------

    - cfm_mean: np.ndarray:
        A numpy array, the mean confusion matrix.
    ...
    for y_pred_i, y_true_i in zip(y_pred, y_true):
        ...
        cfm = skmetrics.confusion_matrix(y_pred=y_pred_i, y_true=y_true_i, **kwargs)
        cfm_list.append(cfm)

    return np.mean(np.stack(cfm_list, axis=0), axis=0)


def std_confusion_matrix(
    y_pred: typing.List[np.ndarray], y_true: typing.List[np.ndarray], **kwargs
) -> np.ndarray:
    """
    This function allows you to calculate the std of
    confusion matrices based on multiple y_pred and y_true arrays.
    ...
    - cfm_std: np.ndarray:
        A numpy array, the std confusion matrix.
    ...
    return np.std(np.stack(cfm_list, axis=0), axis=0)
