...
class BasicModelFitter:
    ...
    def _fit_traditional_batch(self, data):
        ...
        self.writer.add_scalar(
            "Optimiser Time", end_optim_time - start_optim_time, self.step
        )
        self.writer.add_scalar(
            "Forward and Backward Time",
            end_forward_backward_time - start_forward_backward_time,
            self.step,
        )
        ...
        if self.track_accuracy:
            ...
            self.writer.add_scalar(
                "Training Accuracy", correct / len(inputs), self.step
            )
        self.step += 1
    ...
    def _validation(self, val_loader):
        ...
        if self.track_accuracy:
            ...
            self.writer.add_scalar(
                "Validation Accuracy", epoch_validation_accuracy, self.step
            )
    ...
    def _fit_epoch(self, train_loader, val_loader=None):
        ...
        if self.result_save:
            if self.val_too:
                ...
                fig, ax = loss_plot(
                    self.metrics["Epoch Train Loss"], n_epochs=self.n_epochs
                )
            fig.savefig(
                self.result_path + "loss_plot-{}.pdf".format(self.model_name),
                bbox_inches="tight",
            )
            plt.close()
    ...
    def fit(
        ...
        if self.model_save:
            save_name = (
                "{}-epoch_{}".format(self.model_name, epoch + 1) + "-all_trained"
            )
            torch.save(
                self.model.state_dict(),
                self.model_path + save_name + "-state_dict" + ".pth",
            )
